...
Heatmap:
```{r}
png("Swiatkowska_problem1.png",height=700,width=700)
...
```
```{r}
ggplot(PC) + geom_point(aes(x=V2, y=V3, col=as.factor(strain)))  # the best
...
```{r}
PC_loadings = data.table(svd.out$u)
ggplot(PC_loadings) + geom_point(aes(x=V1, y=V2))
```
...
Violin plots:
```{r}
PC_loadings = data.table(svd.out$u[, 1:5]) # select 5 left singular vectors

PC_loadings_melted = melt(PC_loadings, variable.name="PC", value.name="loading")
ggplot(PC_loadings_melted) + geom_violin(aes(x=PC, y=loading))
```
...
ggplot(tsne_out) + geom_point(aes(x = V1, y = V2, color = as.factor(kmeans_result$cluster)))
```
